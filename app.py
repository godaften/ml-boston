...
def predict(): # this function can have any name
    ...
    if in1 == None or in2 == None or in3 == None: # check if any number is missing
        ...
        scaledPredictions = scaler.transform([[float(in1),float(in2),float(in3)]]);
        predictions = model.predict(scaledPredictions) # make new prediction
        #print(scaledPredictions)
        return render_template('index.html', result=str(round(predictions[0][0] * 10000000, 2)))
